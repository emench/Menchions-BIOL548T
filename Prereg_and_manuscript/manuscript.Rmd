...
# PLOTTING WITH BOX PLOTS----
  ...
  ggplot(data=CAL, aes(x=rf_index, y=V_CAL)) + 
...
```{r}
CAL_cover %>% group_by(rf_index) %>% summarise(median = median(cover, na.rm=T)) %>% arrange(desc(median))
```
...
```{r}
RHG_cover %>% group_by(rf_index) %>% summarise(median = median(cover, na.rm=T)) %>% arrange(desc(median))
```
